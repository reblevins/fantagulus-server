...
<div id="main">
	<div id="header">
		...
		<h2 id="blog_sub_title">{{ blogMeta.subTitle }}</h2>
	</div>
	<article v-for="(post, index) in posts" :id="'post_' + post.id" class="mini-post">
		<header>
			<h3><a href="single.html">{{ post.title }}</a></h3>
			<time class="published" datetime="2015-10-17">{{ post.published_date }}</time>
		</header>
		<a href="single.html" class="image"><img :src="post.image"></a>
		<div class="excerpt">
			{{ post.excerpt }}
		</div>
	</article>
